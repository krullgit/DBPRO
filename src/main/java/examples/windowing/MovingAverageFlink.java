...
/**
 * Cleaned version of sliding_window1
 * still on problem left: assign the right time step to data 
 */
public class MovingAverageFlink {
	...
	// format : Tuple<id (maschine, etc.), timestamp, value> 
	// *************************************************************************
	...
	public static void main(String[] args) throws Exception {
		...
		salesData = env.readTextFile(params.get("input")).map(
				new ParseSalesData());
				
		// TODO MArcela pls add windowsize and startTime to params, and show me how it works :D 
		//final int initialWindowSize = Integer.parseInt(args[2]);
		//TODO can we assume that a starttime is known by the user? Otherwise how do we get it? no collect in stream? 
		final int initialWindowSize = 4;
		final int startTime = 1;
		int windowdummy; 
		final int evenOrOdd = initialWindowSize%2;
		if(evenOrOdd==0) windowdummy = initialWindowSize+1;
		
		else windowdummy = initialWindowSize;
		...
		DataStream<Tuple3<Integer, Integer, Double>> topSpeeds = salesData
				...
				// TODO Marcela: If we deal with TimeSeries isn't it that we need 
				// to have an information of the event time? Else we need to apply 
				...
				// how do we do? 
				// TODO MArcela: We can just apply a map-Function and add a random Integer as key
				// to the dataset, I would suggest this as a preproccessing task depending 
				// on the data 
				...
				.apply(new WindowFunction<Tuple3<Integer, Integer, Double>, Tuple3<Integer, Integer, Double>, Tuple, TimeWindow>() {
					...
					public void apply(Tuple arg0, TimeWindow arg1,
						...
						if (test.size() != windowsize) {
							...
							if (test.get(0).f1 == startTime) {
								...
								out.collect(new Tuple3<Integer, Integer, Double>(
										test.get(test.size() - 1).f0, test.get(test
												.size() - 1).f1, test.get(test
												.size() - 1).f2));
								}
								else{
									// do nothing
								...
								out.collect(new Tuple3<Integer, Integer, Double>(
										test.get(0).f0, test.get(0).f1,
										test.get(0).f2));
								}}//borders							
						else{
							// size == middle of list, the point we calculate the
							// mva for
						...
						//even
							...
							double mvaEven = 0.5*(sum1 / (test.size()-1))+0.5*(sum2 / (test.size()-1));
							// if window has enough points
								
							out.collect(new Tuple3<Integer, Integer, Double>(
											test.get(size - 1).f0,
											test.get(size - 1).f1, mvaEven));
							}// end odd 
	...
	private static class ParseSalesData extends
			RichMapFunction<String, Tuple3<Integer, Integer, Double>> {
		private static final long serialVersionUID = 1L;

		@Override
		public Tuple3<Integer, Integer, Double> map(String record) {
			// TODO if data has no key, key here
			// String rawData = record.substring(1, record.length() - 1);
			String rawData = record;
			String[] data = rawData.split(",");
			return new Tuple3<>(Integer.valueOf(data[0].trim()),
					Integer.valueOf(data[1].trim()), Double.valueOf(data[2].trim()));
		}
	}
