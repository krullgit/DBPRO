...
public class TopSpeedWindowing {
	...
	public static void main(String[] args) throws Exception {
		...
		// This will happen using parallelism(1):
		// | point where the window trigger because the accumulated meters reached triggerMeters
		// * the limit of the evictor for every window, a maxby() will be triggered among the elements within the in the last evictSec's
		// 
		// start-------*---|----*---|------------------------------*---|--|-------------------------*---|------end
		//             [   ]    [   ]                              [   ]                            [   ]      <-- in this windows the maxby() is triggered
        //                                                            [   ]  <-- this might happen the limit of the evictor can go beyond the current accumulated window,
		//
		// run with:
		//    parameters: -input ./src/main/resources/car_data_ori.csv <-- original data 
		//    parameters -input ./src/main/resources/car_data1.csv  <-- this works ok, but strage what happen with car_data_ori.csv ???	
		//    parameters -input ./src/main/resources/car_data0.csv  <-- this contains simple timestamps, here there is something strange as well elapsed meters are at some point negatives
		// set:
		//    env.setParallelism(1); <-- so to see what happen more clearly
		//
		// other files to test this in:
		//    
		//
		int evictionSec = 10;  // once the window is collected and the condition is fulfill then the evictor apply the mayxBy() among the element of that last evictionSec's
		double triggerMeters = 50;  // this means that will collect events in the window until the getDelta() value is < triggerMeters
		
		DataStream<Tuple4<Integer, Integer, Double, Long>> topSpeeds = carData
				.assignTimestampsAndWatermarks(new CarTimestamp())
				.keyBy(0)
				.window(GlobalWindows.create()) 
				.evictor(TimeEvictor.of(Time.of(evictionSec, TimeUnit.SECONDS)))   // evictor decide on the number of elements to check before the trigger point to apply the cond maxBy()
				.trigger(DeltaTrigger.of(triggerMeters,
						new DeltaFunction<Tuple4<Integer, Integer, Double, Long>>() {
							private static final long serialVersionUID = 1L;
							private double elapsed_meters = 0.0;
							@Override
							public double getDelta(
									Tuple4<Integer, Integer, Double, Long> oldDataPoint,
									Tuple4<Integer, Integer, Double, Long> newDataPoint) {
								elapsed_meters = newDataPoint.f2 - oldDataPoint.f2;
								System.out.println("       oldDataPoint:" + oldDataPoint.toString() + "-->  newDataPoint:" + newDataPoint.toString() +" ==> elapsed meters:" + elapsed_meters);
