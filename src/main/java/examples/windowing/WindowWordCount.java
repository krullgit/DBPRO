...
import examples.wordcount.util.WordCountData;
import org.apache.flink.streaming.api.datastream.DataStream;
import org.apache.flink.streaming.api.environment.StreamExecutionEnvironment;
import examples.wordcount.WordCount;
...
public class WindowWordCount {
	...
	public static void main(String[] args) throws Exception {
		...
		/**
		 *  if the input is just: "To be, or not to be,--that is the question:-- Whether 'tis nobler in the mind to suffer, The slings ",  
		 *  
			--------------
			windowSize=10
			slideSize=3   <-- it will trigger just the elements that sum = slideSize ???
			.keyBy(0)..countWindow(windowSize, slideSize).sum(1)
			(to,3)
			(the,3)  
