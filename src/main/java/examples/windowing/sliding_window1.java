...
/**
 * Very first draft version of an sliding window to run this use as parameters:
 * -input ./src/main/resources/elecsales_keyed0.csv
 * 
 * 
 * how to work with data like: R/data/elecsales.csv in streamAPI it seems
 * there is no reader for csv -> works see: test.mva.MVA 
 */
public class sliding_window1 {
	...
	public static void main(String[] args) throws Exception {
		...
		// TODO MArcela pls add windowsize to params, and show me how it works :D 
		//final int initialWindowSize = Integer.parseInt(args[2]);
		final int initialWindowSize = 4;
		int windowdummy; 
		if(initialWindowSize%2==0) windowdummy = initialWindowSize+1;
				...
				.apply(new WindowFunction<Tuple3<Integer, Integer, Double>, Tuple3<Integer, Integer, Double>, Tuple, TimeWindow>() {
					...
					public void apply(Tuple arg0, TimeWindow arg1,
						...
						if (test.size() != windowsize) {
							...
							if (test.get(test.size() - 1).f1 < windowsize) {
	...
	private static class ParseSalesData extends
		...
		public Tuple3<Integer, Integer, Double> map(String record) {
			...
			return new Tuple3<>(Integer.valueOf(data[0]),
